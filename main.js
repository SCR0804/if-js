...
console.log(isPalindrome('levem'));
...
// верните массив строк в формате: страна, город, отель;
...
const searchByString = function (arr, str) {
    return arr.filter((item) => `${item.country} ${item.city} ${item.name}`.includes(str))
        .map((item) => `${item.country} ${item.city} ${item.name}`).join('\n');
}

console.log(searchByString(hotels, 'UK'));
console.log(searchByString(hotels, 'Lux'));
...
const uniqCountries = data.reduce((accum, item) => {
        accum[item.country] = accum[item.country] || [];
        accum[item.country].push(item.city);
        return accum;
    }, {})
